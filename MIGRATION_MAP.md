...
| `lib/highlighter/` | 2 | becomes a Vite `?worker` import target with the diff UI | `rdc/src/lib/highlighter/` | Phase 7f — bounded plain-text diffs satisfy the MVP |
...
| `lib/editors/**` | `src-tauri/src/platform/editors.rs` + `commands/platform/editor.rs`; typed wrapper at `src/platform/editors.ts` | **Phase 4 complete on Linux/macOS** — discovery, path validation, and normal/custom launch landed test-first; Windows is Phase 10 |
...
| `lib/main-process-config.ts` | `src-tauri/src/config.rs` + `src-tauri/src/commands/platform/config.rs` + `src/platform/config.ts` | **Phase 4 complete** — startup `titleBarStyle`, typed get/serialized partial update, and live `hideWindowOnQuit` close policy |
...
| `commit.ts` | create/amend and merge commits → `commit.rs`; hook progress and combined commit stdout/stderr cross through Channels | backend and Channel handoff **complete**; bounded frontend history, enable state and shared commit progress dialog **complete** |
...
| `hooks-proxy.ts` | proxy transport/runner/server → `hooks/{protocol,client,server,runner}.rs` + `rdc-hook-proxy` | backend and Phase 3 command Channel handoff **complete**; minimum failure UI **Phase 7b complete**, richer progress UI Phase 7f |
...
| `notifications.ts` | `src-tauri/src/platform/notification.rs` + `src-tauri/src/commands/platform/notification.rs` + `src/platform/notifications.ts` — **Phase 4b complete** with one Rust click router and a retained `notify-rust` handle; native evidence follows an exposed Phase 8b MVP consumer or the Phase 9/10 release targets | 4 / 8b / 9 / 10 |
...
| _(new)_ | request/response | `resolve_hook_failure` → `resolveHookFailure()` | **done** — answers an ID-scoped failed-hook prompt with abort or ignore; missing callbacks fail closed |
...
| Query/cancel commands | `src-tauri/src/commands/operations.rs` |
