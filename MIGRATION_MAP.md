...
| `lib/git/diff.ts` (`getBinaryPaths` + its two private helpers only) | `crates/git-ops/src/diff.rs` — **partial, deliberately**: 1,032-line module, and `status` needs only these. Diff rendering/image diffs/blob fetching land with the diff UI. **Contains an upstream bug fix — see §8.** | 2 |
...
| `lib/git/branch.ts` | `crates/git-ops/src/branch.rs` — **partially done**: `create_branch`, `get_branch_names`, `rename_branch` (incl. the case-only-rename retry), `delete_local_branch`, `get_branches_pointed_at`, `get_merged_branches`. **`deleteRemoteBranch` deferred** — it pushes a deletion, needing `envForRemoteOperation` (= `envForAuthentication` trampoline/askpass + `envForProxy`), both outstanding. | 2 |
