...
| `discardAll` | Whole-tree discard (tracked + untracked asymmetry) | One modified tracked `trackedFile` and one untracked `untrackedFile`. After Discard All, the tracked file's bytes equal `expectedTrackedContent` (tracked modifications are unrecoverable) and the untracked file no longer exists on disk. |
| `discardMany99` | Discard-all confirmation at 99 files | 99 changed files: 49 tracked modifications under nested `src/module-NNNN/` paths, 50 untracked under `generated/deeply/nested/`. Stays **under** `VirtualList`'s virtualization threshold of 100, so every row is genuinely in the DOM. |
| `discardMany1000` | Discard-all confirmation at 1000 files, and discard throughput | 1000 changed files, same tracked/untracked split and nesting. **Crosses** the threshold, so the list is windowed. Also the only fixture that exercises a large batched removal end to end. |
| `mergeStates` | The merge dialog's three outcomes | `main` plus three branches, each in a state the dialog treats differently: `already-merged` is contained in `main` and must be **absent** from the candidate list; `clean-merge` is 2 commits ahead with no overlap and must report that count; `conflicting-merge` edits the same line `main` later edited and must warn about 1 conflicted file. The debug menu's stub branches cannot produce any of this — mergeability is computed by git from the repository, so a branch that exists only in the renderer has no state at all. |
...
| `multiWindowPushA` | Multi-window operation ownership — see `multi-window-checklist.md` | Same unpublished-branch topology as `remotePush`, with a `delaySeconds` of 20 rather than `delayedPush`'s 3. The longer window is the point: the rows it serves require switching windows, reading both, and closing one while the operation runs, none of which fits in three seconds. |
| `multiWindowPushB` | A **second, independent** repository for the concurrent-operations row | Identical topology to `multiWindowPushA`, with its own working tree and its own bare remote. Two windows must be able to run operations at once without contending for one repository lock — sharing a repository would test the opposite of what that row asks. |
