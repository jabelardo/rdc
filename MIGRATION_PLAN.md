...
progress dialog is consumed by Clone, Rebase, Commit, Merge and — after the operation registry made
them repository-locking — Fetch, Push and Pull as well. Checkout is the one embedded consumer, in
the branch sidebar, rendering the same shared body without the modal wrapper. History-operation
...
[`OPERATION_PROGRESS_PLAN.md`](./OPERATION_PROGRESS_PLAN.md), **complete as of Slice 20 on
2026-08-15**. It superseded the old assumption that blocking progress can never offer an abort
control, while preserving the multi-window rule that operations in one local repository must not
block windows showing another.

**What it landed, and what was measured rather than asserted.** A native registry owns every
operation's lifetime, progress, cancellation capability, timeout and recovery state, with locks
scoped to the repository rather than the application. All ten operation kinds now expose
cancellation — Fetch, Push, Pull, Checkout, Clone, Commit, Merge, Rebase, CherryPick and Revert —
each having first proven process-tree termination, post-cancel inspection and a recoverable error
event. Push's control reads "Stop waiting" rather than "Cancel push", and it finishes with an
explicitly unknown outcome, because killing the local process cannot un-send what the remote may
...
roles, compact viewport, and the shared body embedded rather than wrapped) and in the new
`qa/phase-8b/multi-window-checklist.md`, whose fixtures — `multiWindowPushA` and `multiWindowPushB`,
two independent repositories with a twenty-second push window — are generated by
`pnpm fixture:phase8b`.
