...
describe("large lists", () => {
  ...
  it("bounds representative large repository and change lists while preserving End navigation", async () => {
    for (let index = 0; index < largeFileCount; index++) {
      writeFileSync(
        path.join(fixture.canonical, `large-${String(index).padStart(4, "0")}.txt`),
        `large fixture ${index}\n`,
      );
    }
    ...
    const changedList = await driver.wait(
      until.elementLocated(By.css('[aria-label="Changed files"][data-virtualized="true"]')),
      10_000,
    ...
    const repositoryList = await driver.wait(
      until.elementLocated(By.css('[aria-label="Repositories"][data-virtualized="true"]')),
      10_000,
    ...
    assert.ok(
      (await changedList.findElements(By.css("[data-changed-file-path]"))).length < 40,
      "the thousand-file fixture rendered an unbounded DOM list",
    );
    assert.ok(
      (await repositoryList.findElements(By.css(".repository-list-item"))).length < 40,
    ...
    await driver.wait(
      until.elementLocated(By.css(`[data-changed-file-path="${lastPath}"] [aria-current="true"]`)),
      5_000,
