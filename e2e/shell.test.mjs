...
import { resetRepositoryFixtures, sendNativeKeys, startApplication } from "./harness.mjs";
...
describe("application shell", () => {
  ...
  it("launches the real Tauri application", async () => {
    ...
    assert.equal(
      await driver
        .findElements(By.css('main.application-shell [aria-label="Navigation"]'))
  ...
  it("starts with every sidebar panel collapsed", async () => {
    const headings = await driver.executeScript(() =>
      [...document.querySelectorAll(".sidebar-panels h2 button")].map((button) => ({
        id: button.id,
  ...
  it("keeps the empty shell aligned through sidebar collapse", async () => {
    const measure = () =>
      driver.executeScript(() => {
        ...
        return {
          ...
          commandBarRightPadding: Number.parseFloat(getComputedStyle(commandBar).paddingRight),
          collapseCenterOffset:
            collapseRect.left + collapseRect.width / 2 - (sidebarRect.left + sidebarRect.width / 2),
          ...
          railSectionHeight: railSection?.getBoundingClientRect().height ?? null,
    ...
    assert.ok(
      Math.abs(expanded.collapseRightGap - expanded.commandBarRightPadding) <= 1,
    ...
    await driver.findElement(By.css('button[aria-label="Collapse sidebar"]')).click();
    await driver.wait(until.elementLocated(By.css('button[aria-label="Expand sidebar"]')), 5_000);
    ...
    await driver.findElement(By.css('button[aria-label="Expand sidebar"]')).click();
    await driver.wait(until.elementLocated(By.css('button[aria-label="Collapse sidebar"]')), 5_000);
  });
  ...
  it("writes configuration and logs only to the isolated application directories", async () => {
    ...
    const logPath = path.join(process.env.XDG_DATA_HOME, "org.rdc", "logs", "RDC.log");
    await driver.wait(
      () => existsSync(logPath),
      5_000,
  ...
  it("opens and dismisses the add-repository dialog from the application menu", async () => {
    ...
    await driver.wait(
      until.elementLocated(By.xpath("//button[normalize-space()='Create repository']")),
      5_000,
    );
  });
  ...
  it("opens MVP preferences from the native menu and applies theme changes", async () => {
    ...
    await driver.wait(
      async () => (await driver.findElement(By.css("html")).getAttribute("data-theme")) === "dark",
      5_000,
    ...
    await driver.wait(until.stalenessOf(preferences), 5_000, "preferences dialog did not close");
  });
});
