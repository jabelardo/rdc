...
describe("visual layout", () => {
  ...
  before(async () => {
    ...
    writeFileSync(path.join(fixture.canonical, "gate-c-layout.txt"), "Gate C layout fixture\n");
  ...
  it("keeps the repository command bar aligned and usable at compact width", async () => {
    const normal = await driver.executeScript(() => {
      ...
      return {
        ...
        sidebarCommandBarHeight: sidebarCommandBar.getBoundingClientRect().height,
    ...
    await driver.wait(
      async () =>
        (await driver.findElement(By.css(".app-tooltip")).getText()) === "Collapse sidebar",
      2_000,
    ...
    try {
      ...
      assert.ok(minimumRect.width >= 715, "window width escaped its 715px floor");
      assert.ok(minimumRect.height >= 356, "window height escaped its 356px floor");
      await driver.wait(
        async () => await driver.executeScript(() => matchMedia("(max-width: 46rem)").matches),
        5_000,
      ...
      const compactSnapshot = async () =>
        await driver.executeScript(() => {
          ...
          const viewLabels = [...document.querySelectorAll(".repository-view-label")];
          return {
      ...
      await driver.executeScript(() => document.querySelector(".sidebar-collapse").click());
      await driver.wait(
        async () =>
          await driver.executeScript(() =>
            document.querySelector(".application-shell").classList.contains("sidebar-collapsed"),
          ),
        5_000,
  ...
  it("keeps compact controls and file statuses visually semantic in Dark", async () => {
    await driver.executeScript(() => {
      ...
      const branches = document.querySelector('[aria-controls="sidebar-branches"]');
    ...
    await driver.wait(
      async () =>
        await driver.executeScript(() => document.querySelector(".new-branch-button") !== null),
      5_000,
    ...
    await driver.executeScript(() =>
      document.querySelector('.repository-view-navigation [aria-label="History"]').click(),
    );
    await driver.wait(
      async () =>
        await driver.executeScript(() => document.querySelector(".history-file-status") !== null),
      5_000,
    ...
    await driver.executeScript(() => {
      document.querySelector('.repository-view-navigation [aria-label="Changes"]').click();
      const repositories = document.querySelector('[aria-controls="sidebar-repositories"]');
  ...
  it("keeps every Changes region bounded and reachable at compact width", async () => {
    ...
    try {
      ...
      const layout = await driver.executeScript(() => {
        ...
        const separators = [...document.querySelectorAll(".horizontal-resizer")].filter(
          (separator) => separator.getBoundingClientRect().height > 0,
        );
        ...
        return {
          workspaceOwnsOverflow: getComputedStyle(workspace).overflowY === "hidden",
          ...
          filesScrollIndependently:
            getComputedStyle(files.querySelector(".virtual-list-viewport")).overflowY === "auto",
          diffScrollsIndependently:
            getComputedStyle(diff.querySelector(".working-tree-diff-content")).overflowY === "auto",
  ...
  it("keeps History side by side with independently bounded regions", async () => {
    ...
    try {
      ...
      const sidebarWidthBeforeViewSwitch = await driver.executeScript(
        () => document.querySelector(".repository-sidebar").getBoundingClientRect().width,
      );
      await driver.executeScript(() =>
        document.querySelector('.repository-view-navigation [aria-label="History"]').click(),
      );
      ...
      await driver.wait(
        async () =>
          await driver.executeScript(
            () =>
              document.querySelector('.history-files [aria-current="true"]') !== null &&
      ...
      const snapshot = () =>
        driver.executeScript(() => {
          ...
          const selectedCommit = history.querySelector('.history-commits [aria-current="true"]');
          const selectedFile = history.querySelector('.history-files [aria-current="true"]');
          const separators = [...document.querySelectorAll(".horizontal-resizer")].filter(
            (separator) => separator.getBoundingClientRect().height > 0,
          );
          ...
          return {
            columns: getComputedStyle(history).gridTemplateColumns.split(" ").length,
            historyOwnsOverflow: getComputedStyle(history).overflowY === "hidden",
            historyFits: history.scrollHeight <= history.clientHeight,
            listStaysLeftOfDetails: commitRect.right <= detailsRect.left + 1,
            filesStayLeftOfDiff: fileSectionRect.right <= diffRegionRect.left + 1,
            regionsFit: [commits, details, metadata, fileSection, files, diffRegion, diff].every(
              regionFits,
            ),
            commitListScrollsIndependently: getComputedStyle(commits).overflowY === "auto",
            fileListScrollsIndependently: getComputedStyle(files).overflowY === "auto",
            diffScrollsIndependently: getComputedStyle(diff).overflowY === "auto",
      ...
      assert.deepEqual(
        {
          ...
          commitListScrollsIndependently: beforeSwitch.commitListScrollsIndependently,
          fileListScrollsIndependently: beforeSwitch.fileListScrollsIndependently,
      ...
      assert.ok(
        Math.abs(sidebarWidthAfterViewSwitch - sidebarWidthBeforeViewSwitch) <= 1,
      ...
      const resizedWidths = await driver.executeScript(() => ({
        commits: document.querySelector(".history-list-pane").getBoundingClientRect().width,
        files: document.querySelector(".history-file-section").getBoundingClientRect().width,
      }));
      ...
      await driver.wait(
        async () =>
          await driver.executeScript(() => !document.querySelector(".changes-workspace").hidden),
        5_000,
      ...
      await driver.wait(
        async () => await driver.executeScript(() => !document.querySelector(".history").hidden),
        5_000,
  ...
  it("keeps the settled frames stable through continuous resize and restore", async () => {
    ...
    const selectView = async (view) => {
      await driver.executeScript((selectedView) => {
        document
          .querySelector(`.repository-view-navigation [aria-label="${selectedView}"]`)
          .click();
      }, view);
      await driver.wait(
        async () =>
          await driver.executeScript((selectedView) => {
            const selector = selectedView === "Changes" ? ".changes-workspace" : ".history";
    ...
    const snapshot = () =>
      driver.executeScript(() => {
        ...
        const common = {
          ...
          workspaceStartsBelowToolbar: Math.abs(workspaceRect.top - toolbarRect.bottom) <= 1,
          dragRegionFits:
            dragRegion === null ||
            (dragRegion.getBoundingClientRect().left >= -1 &&
              dragRegion.getBoundingClientRect().right <= window.innerWidth + 1),
        };
        ...
        if (selectedView === "Changes") {
          ...
          return {
            ...
            paneMinimumsHold: filesRect.width >= 190 - 1 && diffRect.width >= 300 - 1,
            selectedCommit: null,
        ...
        return {
          ...common,
          paneDirectionStable:
            commits.getBoundingClientRect().right <= details.getBoundingClientRect().left + 1 &&
            files.getBoundingClientRect().right <= diff.getBoundingClientRect().left + 1,
          workspaceFits:
            workspace.scrollWidth <= workspace.clientWidth &&
            workspace.scrollHeight <= workspace.clientHeight,
          paneMinimumsHold:
            commitsRect.width >= 190 - 1 && filesRect.width >= 150 - 1 && diffRect.width >= 220 - 1,
      ...
      for (const view of ["Changes", "History"]) {
        ...
        if (view === "Changes") {
          const hasSelection = await driver.executeScript(
            () => document.querySelector('.working-tree-files [aria-current="true"]') !== null,
          );
          if (!hasSelection) {
            await driver.executeScript(() =>
        ...
        for (const collapsed of [false, true]) {
          ...
          for (const width of widths) {
            ...
            const currentSelection = `${state.selectedCommit ?? ""}|${state.selectedFile ?? ""}`;
            if (selections.has(selectionKey)) {
              ...
              assert.notEqual(currentSelection, "|", `${view} had no selection before resizing`);
  ...
  it("gives one sidebar panel the remaining height without hiding sibling headers", async () => {
    ...
    const snapshot = () =>
      driver.executeScript(() => {
        ...
        const repositoriesHeading = document.querySelector("#sidebar-repositories-heading");
        const branchesHeading = document.querySelector("#sidebar-branches-heading");
        ...
        const repositoriesHeadingRect = repositoriesHeading.getBoundingClientRect();
        const branchesHeadingRect = branchesHeading.getBoundingClientRect();
        return {
          repositoriesExpanded: repositoriesHeading.getAttribute("aria-expanded") === "true",
          branchesExpanded: branchesHeading.getAttribute("aria-expanded") === "true",
          ...
          expandedRegionHeight: (repositories ?? branches)?.getBoundingClientRect().height ?? 0,
        };
      });
    ...
    await driver.executeScript(() => document.querySelector("#sidebar-branches-heading").click());
