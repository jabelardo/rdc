...
describe("getCloneDirectoryName", () => {
  it("uses the parsed repository name for hosted URLs", () => {
    expect(getCloneDirectoryName("https://example.com/org/repository.git")).toBe("repository");
    expect(getCloneDirectoryName("git@example.com:org/repository.git")).toBe("repository");
  });
