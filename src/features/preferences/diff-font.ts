import { getInstalledMonospaceFontFamilies } from "@/features/preferences/fonts/monospace-font-filter";
...
export const availableDiffFontSizes: ReadonlyArray<number> = [10, 11, 12, 13, 14, 16];
...
let availableDiffFontFamiliesPromise: Promise<ReadonlyArray<DiffFontFamily>> | null = null;

export async function getAvailableDiffFontFamilies(): Promise<ReadonlyArray<DiffFontFamily>> {
  ...
  availableDiffFontFamiliesPromise = (async () => {
    ...
    return [defaultDiffFontFamily, ...families.map((f) => `${localFontPrefix}${f}`)];
  })();
