...
import { Tokenizer, TokenType, EmojiMatch, HyperlinkMatch } from "./text-token-parser";
...
describe("Tokenizer", () => {
  ...
  describe("with GitHub repository", () => {
    ...
    it("does not render link when email address found", () => {
      ...
      assert.equal(results.length, 1);
      assert.equal(results[0].kind, TokenType.Text);
      assert.equal(results[0].text, "the email address support@github.com should be ignored");
    });
    ...
    it("renders link and author mention when parsing release notes", () => {
      ...
      assert.equal(results[0].kind, TokenType.Text);
      assert.equal(results[0].text, `'Clone repository' menu item label is obscured on Windows - `);
...
Signed-off-by: Johannes Schindelin <johannes.schindelin@gmx.de>`;
      ...
      assert.equal(issue.url, "https://github.com/shiftkey/some-repo/issues/1034");
...
Signed-off-by: Johannes Schindelin <johannes.schindelin@gmx.de>`;
      ...
      assert.equal(mention.text, "https://github.com/shiftkey/some-repo/issues/1034");
      assert.equal(mention.url, "https://github.com/shiftkey/some-repo/issues/1034");
    });
  });
});
