...
describe("getAbsoluteUrl", () => {
  describe("dotcom endpoint", () => {
    ...
    it("doesn't mangle encoded query parameters", () => {
      ...
      assert.equal(result, "https://api.github.com/issues?since=2019-05-10T16%3A00%3A00Z");
    });
  });
  ...
  describe("enterprise endpoint", () => {
    ...
    it("handles next page resource which already contains prefix", () => {
      const result = getAbsoluteUrl(enterpriseEndpoint, "/api/v3/user/repos?page=2");
      assert.equal(result, `${enterpriseEndpoint}/user/repos?page=2`);
    });

    it("doesn't mangle encoded query parameters", () => {
      const result = getAbsoluteUrl(enterpriseEndpoint, "/issues?since=2019-05-10T16%3A00%3A00Z");
      assert.equal(result, `${enterpriseEndpoint}/issues?since=2019-05-10T16%3A00%3A00Z`);
    });
  });
});
