...
function normalizeTemplate(template: ReadonlyArray<TemplateItem>): IMenu {
  ...
  const items = (
    source: ReadonlyArray<TemplateItem>,
    prefix = '@'
...
export function buildDefaultMenu(
  ...
  const pullRequestLabel = hasCurrentPullRequest
    ? `${mac ? 'View Pull Request ' : 'View &pull request '}${onGithubLabel(
        gitHubRepositoryType
      )}`
    : mac
      ? 'Create Pull Request'
  ...
  template.push({
    ...
    submenu: [
      ...
      separator(),
      {
        ...
        label: `${mac ? 'View ' : '&View '}${onGithubLabel(
          gitHubRepositoryType
        )}`,
      ...
      separator(),
      {
        ...
        label: `${mac ? 'Create Issue ' : 'Create &issue '}${onGithubLabel(
          gitHubRepositoryType
        )}`,
  ...
  template.push({
    ...
    submenu: [
      ...
      {
        label: `${mac ? 'View Branch ' : 'View branch '}${onGithubLabel(
          gitHubRepositoryType
        )}`,
        id: 'branch-on-github',
