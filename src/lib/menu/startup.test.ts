...
describe("startup default menu", () => {
  it("keeps supported and native actions enabled while disabling unconnected UI actions", () => {
    ...
    expect(
      items.find((item) => item.type === "menuItem" && item.action?.type === "show-logs"),
    ).toMatchObject({ enabled: true });
    expect(
      items.find((item) => item.type === "menuItem" && item.action?.type === "open-external"),
    ).toMatchObject({ enabled: true });
    expect(items.find((item) => item.type === "menuItem" && item.role === "copy")).toMatchObject({
      enabled: true,
    });
  });
});
...
describe("startup menu actions", () => {
  it("executes every action the startup menu leaves enabled", async () => {
    ...
    await expect(execute({ type: "open-external", url: "https://example.com" })).resolves.toBe(
      true,
    );
    await expect(execute({ type: "menu-event", event: "select-all" })).resolves.toBe(true);
    await expect(execute({ type: "zoom", direction: "in" })).resolves.toBe(true);
    await expect(execute({ type: "zoom", direction: "out" })).resolves.toBe(true);
    await expect(execute({ type: "zoom", direction: "reset" })).resolves.toBe(true);
  ...
  it("refuses actions that must wait for the Phase 7 dispatcher", async () => {
    ...
    await expect(execute({ type: "menu-event", event: "pull" })).resolves.toBe(false);
