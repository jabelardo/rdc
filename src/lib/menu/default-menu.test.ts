...
describe.each<MenuPlatform>(["macos", "windows", "linux"])("%s default menu", (platform) => {
...
describe("platform and state-derived menu structure", () => {
  ...
  it("uses only rdc-owned Help destinations and product identity", () => {
    for (const platform of ["macos", "windows", "linux"] as const) {
      ...
      expect(
        items.some(
          (item) => item.type !== "separator" && item.label.includes("Keyboard Shortcuts"),
        ),
      ).toBe(false);
      if (platform !== "macos") {
  ...
  it("truncates the contribution target and derives repository labels", () => {
    ...
    expect(labelById(items, "update-branch-with-contribution-target-branch")).toBe(
      "&Update from a-very-long-contribution-…",
    );
    expect(items.find((item) => item.id === "push")).toMatchObject({
      label: "Force P&ush…",
      action: { type: "menu-event", event: "force-push" },
    });
    expect(labelById(items, "view-repository-on-github")).toBe("&View on GitLab");
  });
});
