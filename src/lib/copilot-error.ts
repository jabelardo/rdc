...
export class CopilotError extends Error {
  ...
  public constructor(message: string, statusCode: number, options: ICopilotErrorOptions = {}) {
    super(message);
...
function isPaymentRequiredErrorCode(value: unknown): value is CopilotPaymentRequiredErrorCode {
  return typeof value === "string" && knownPaymentRequiredErrorCodes.some((code) => code === value);
}
...
export function getCopilotPaymentRequiredErrorFromSessionError(data: {
  ...
  const code = isPaymentRequiredErrorCode(data.errorCode) ? data.errorCode : undefined;
  const cleaned = cleanSessionErrorMessage(data.message ?? "", data.statusCode);
  const message = cleaned.length > 0 ? cleaned : getFallbackPaymentRequiredMessage(code);
 ...
 */
export function cleanSessionErrorMessage(message: string, statusCode: number): string {
  return message
...
function getFallbackPaymentRequiredMessage(code: CopilotPaymentRequiredErrorCode | undefined) {
  switch (code) {
    case "quota_exceeded":
    ...
    try {
      ...
      if (isRecord(parsed)) {
        ...
        if (isRecord(error)) {
          ...
          } else if (topLevelMessage !== undefined && topLevelMessage.trim().length > 0) {
...
export function getCopilotErrorDisplayInfo(error: CopilotError): ICopilotErrorDisplayInfo | null {
  ...
  switch (error.code) {
    case "quota_exceeded":
      return {
        ...
        retryAfterMessage:
          error.retryAfter !== undefined ? getRetryAfterMessage(error.retryAfter) : undefined,
      };
