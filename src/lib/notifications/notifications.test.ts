...
describe("notification callbacks", () => {
  ...
  it("stores a callback only after native display returns an id", async () => {
    ...
    await showNotification({ title: "Unavailable", body: "No native backend", onClick }, callbacks);

    expect(nativeShowNotification).toHaveBeenNthCalledWith(1, "Review", "A comment arrived", {
      type: "pr-comment",
    });
  ...
  it("focuses and consumes the matching callback exactly once", async () => {
    ...
    let listener:
      | ((event: "click", id: string, userInfo: Record<string, unknown> | undefined) => void)
      | undefined;
