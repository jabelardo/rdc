...
describe("wrapRichTextCommitMessage", () => {
  ...
  it("doesn't wrap at exactly 72 chars", async () => {
    const summaryText = "weshouldnothardwrapthislongsummarywhichisexactly72charactersyeswetotally";
  ...
  it("wraps links properly", async () => {
    ...
    assert.equal(summary[0].kind, TokenType.Text);
    assert.equal(summary[0].text, "Link should be truncated but open our release notes ");

    assert.equal(summary[1].kind, TokenType.Link);
    assert.equal(summary[1].text, "https://desktop.gith");
    assert.equal((summary[1] as HyperlinkMatch).url, "https://desktop.github.com/release-notes/");
    ...
    assert.equal((body[1] as HyperlinkMatch).url, "https://desktop.github.com/release-notes/");
  });
});
