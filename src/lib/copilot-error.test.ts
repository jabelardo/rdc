...
describe("parseCopilotPaymentRequiredError", () => {
  it("parses quota_exceeded responses", () => {
    ...
    assert.equal(error.message, "You have used all available Copilot premium requests.");
  ...
  it("parses session_quota_exceeded responses", () => {
    ...
    assert.equal(error.message, "You have reached the session limit for Copilot requests.");
  ...
  it("parses billing_not_configured responses", () => {
    ...
    assert.equal(error.message, "Configure billing in GitHub Settings to continue.");
    assert.equal(error.code, "billing_not_configured");
  });

  it("falls back to the raw response body when the server returns plain text", () => {
    const error = parseCopilotPaymentRequiredError("You have reached your quota limit.", null);
...
describe("getCopilotErrorDisplayInfo", () => {
  it("returns a quota-specific title and retry timing for quota_exceeded", () => {
    ...
    assert.equal(displayInfo?.message, "You have used all available Copilot premium requests.");
  ...
  it("returns a distinct title for session_quota_exceeded", () => {
    ...
    assert.equal(displayInfo?.message, "You have reached the session limit for Copilot requests.");
    assert.equal(displayInfo?.actionText, undefined);
  });
  ...
  it("returns a settings CTA for billing_not_configured", () => {
    ...
    assert.equal(displayInfo?.message, "Configure billing in GitHub Settings to continue.");
...
describe("getCopilotPaymentRequiredErrorFromSessionError", () => {
  ...
  it("falls back to a default message when the cleaned message is empty", () => {
    ...
    assert.equal(error?.message, "GitHub Copilot billing is not configured for this account.");
    assert.equal(error?.code, "billing_not_configured");
  });
