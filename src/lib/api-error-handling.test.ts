...
import { APIError, getAbsoluteUrl } from "./http";
...
describe("http", () => {
  ...
  describe("getAbsoluteUrl", () => {
    ...
    it("strips duplicate api/v3/ prefix from path", () => {
      const url = getAbsoluteUrl("https://ghe.example.com/api/v3/", "api/v3/repos/owner/repo");
      assert.equal(url, "https://ghe.example.com/api/v3/repos/owner/repo");
    });

    it("handles enterprise endpoints", () => {
      const url = getAbsoluteUrl("https://ghe.example.com/api/v3", "/repos/owner/repo");
      assert.equal(url, "https://ghe.example.com/api/v3/repos/owner/repo");
    });
  });
});
