...
import { getNextPagePathWithIncreasingPageSize } from "./api";
...
function assertNext(current: IPageInfo, expected: IPageInfo) {
  ...
  const per_page = parseInt(typeof query.per_page === "string" ? query.per_page : "", 10);
...
describe("API", () => {
  describe("getNextPagePathWithIncreasingPageSize", () => {
    it("returns null when there's no link header", () => {
      assert(
        getNextPagePathWithIncreasingPageSize(new Response(), "per_page", "page", 100) === null,
      );
    });
