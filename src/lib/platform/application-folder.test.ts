...
const { isInApplicationFolder, moveToApplicationsFolder } = await import("./application-folder");
...
describe("macOS application-folder commands", () => {
  it("preserves the nullable query and move contracts", async () => {
    ...
    expect(invoke).toHaveBeenNthCalledWith(1, "is_in_application_folder");
    expect(invoke).toHaveBeenNthCalledWith(2, "move_to_applications_folder");
  });
});
