...
export class AppStore {
  ...
  public async addRepository(requestedPath: string, persistSelection = true): Promise<Repository> {
    const type = await this.dependencies.getRepositoryType(requestedPath);
    if (type.kind !== "regular") {
      throw new Error(`${requestedPath} isn't a Git working repository (${type.kind}).`);
    }
    ...
    this.repositories = await this.repositoriesStore.getAll();
    const repository = this.repositories.find((candidate) => candidate.id === added.id) ?? added;
  ...
  public async removeRepository(repository: Repository): Promise<void> {
    ...
    const selected =
      this.selectedRepository?.id === repository.id
        ...
        : (this.repositories.find((candidate) => candidate.id === this.selectedRepository?.id) ??
          this.repositories[0] ??
          null);
    await this.selectRepository(selected);
  }
  ...
  public async selectRepository(
    ...
    const selected =
      repository === null
        ? null
        : (this.repositories.find((candidate) => candidate.id === repository.id) ?? null);
