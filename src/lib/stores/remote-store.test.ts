...
describe("RemoteStore", () => {
  ...
  it("ignores a slow load after the repository changes", async () => {
    ...
    const getRemotes = vi.fn().mockReturnValueOnce(oldRemotes).mockResolvedValueOnce([upstream]);
    const store = new RemoteStore({
      getRemotes,
      getBranches: vi.fn(async () => [branch("main", null)]),
      getStatus: vi.fn(async () => ({ currentBranch: "main" })),
  ...
  it("adds a remote and refreshes the remote facts", async () => {
    ...
    const getRemotes = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([origin]);
    const store = new RemoteStore({
      getRemotes,
      getBranches: vi.fn(async () => [branch("main", null)]),
      getStatus: vi.fn(async () => ({ currentBranch: "main" })),
    ...
    await expect(store.addRemote("origin", "/remotes/origin.git")).resolves.toBe(true);

    expect(addRemote).toHaveBeenCalledWith("/repo", "origin", "/remotes/origin.git");
  ...
  it("removes a remote and refreshes the remote facts", async () => {
    ...
    const getRemotes = vi.fn().mockResolvedValueOnce([origin]).mockResolvedValueOnce([]);
    const store = new RemoteStore({
      getRemotes,
      getBranches: vi.fn(async () => [branch("main", null)]),
      getStatus: vi.fn(async () => ({ currentBranch: "main" })),
