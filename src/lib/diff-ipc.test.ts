...
describe('the diff wire shape', () => {
  ...
  it('agrees with Rust on every hunk expansion type', () => {
    ...
    diff.hunks.forEach((hunk, index) => {
      ...
      expect(
        getHunkHeaderExpansionType(index, hunk.header, previous),
        `hunk ${index}`
      ).toBe(hunk.expansionType)
    })
  ...
  it('agrees with Rust that the fixture has no hidden bidi characters', () => {
    expect(HiddenBidiCharsRegex.test(parsedDiff.contents)).toBe(
      parsedDiff.hasHiddenBidiChars
    )
  })
