...
describe("parsePACString", () => {
  ...
  it("parses Chromium PAC strings", () => {
    assert.deepStrictEqual(parsePACString("PROXY myproxy:80;DIRECT"), ["http://myproxy:80"]);

    assert.deepStrictEqual(parsePACString("PROXY myproxy:80"), ["http://myproxy:80"]);
    assert.deepStrictEqual(parsePACString("PROXY myproxy:80; HTTPS secureproxy:443"), [
      "http://myproxy:80",
      "https://secureproxy:443",
    ]);

    assert.deepStrictEqual(
      parsePACString("PROXY a:1;HTTP b:2;HTTPS c:3;SOCKS d:4;SOCKS4 e:5;SOCKS5 f:5;DIRECT"),
      ["http://a:1", "http://b:2", "https://c:3", "socks4://d:4", "socks4://e:5", "socks5://f:5"],
    );
  });
