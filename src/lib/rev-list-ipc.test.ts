...
function branch(name: string, upstream: string | null, type = BranchType.Local): Branch {
  return new Branch(
    name,
    upstream,
...
describe("getBranchAheadBehind", () => {
  ...
  it("answers null for a local branch with no upstream without asking git", async () => {
    await expect(getBranchAheadBehind(REPO, branch("local-only", null))).resolves.toBeNull();
