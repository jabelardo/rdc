...
const pushProgressInitial: IPushProgress = snapshot.pushProgressInitial as IPushProgress;
...
describe("the remote commands", () => {
  ...
  it("fastForwardBranches sends pairs and no channel", async () => {
    ...
    await fastForwardBranches(REPO, [["refs/remotes/origin/main", "refs/heads/main"]]);
...
describe("the clone and remote commands", () => {
  ...
  it("getRemoteURL distinguishes a missing remote with null", async () => {
    ...
    await expect(getRemoteURL(REPO, "origin")).resolves.toBe("https://github.com/o/r.git");
  });
