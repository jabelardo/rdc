...
describe("application menu controller", () => {
  ...
  it("rejects disabled, missing, actionless, and stale items", async () => {
    ...
    await expect(controller.executeItemById("does-not-exist")).resolves.toBe(false);
  ...
  it("cleans up both macOS listeners when native installation fails", async () => {
    ...
    vi.mocked(deps.setNativeMenu).mockRejectedValue(new Error("native menu failed"));

    await expect(installApplicationMenu({}, deps)).rejects.toThrow("native menu failed");
