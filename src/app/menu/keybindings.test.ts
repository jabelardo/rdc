...
describe("structured keybinding matching", () => {
  it("matches physical code and every modifier exactly", () => {
    expect(
      matchesKeybinding(keyboardEvent("KeyQ", { ctrlKey: true, shiftKey: true }), binding),
    ).toBe(true);
    expect(
      matchesKeybinding(
        keyboardEvent("KeyQ", {
    ...
    expect(
      matchesKeybinding(keyboardEvent("KeyA", { ctrlKey: true, shiftKey: true }), binding),
    ).toBe(false);
  });
  ...
  it("returns only executable, enabled, visible menu items", () => {
    ...
    expect(
      findMenuItemForKeybinding(keyboardEvent("KeyP", { ctrlKey: true }), appMenu, bindings),
    ).toBeUndefined();
  });
  ...
  it("installs a capture-phase dispatcher and returns its cleanup", () => {
    ...
    expect(execute).toHaveBeenCalledWith(expect.objectContaining({ id: "pull" }));
...
describe("friendly keybinding text", () => {
  it("renders platform modifier names from structured data", () => {
    ...
    expect(friendlyKeybindingText({ modifiers: ["alt", "meta"], key: "KeyI" }, "macos")).toBe(
      "⌥⌘I",
    );
  });

  it("renders physical punctuation and named keys", () => {
    expect(friendlyKeybindingText({ modifiers: ["control"], key: "Comma" }, "linux")).toBe(
      "Ctrl+,",
    );
    expect(friendlyKeybindingText({ modifiers: ["meta"], key: "Backspace" }, "macos")).toBe("⌘⌫");
  });
});
