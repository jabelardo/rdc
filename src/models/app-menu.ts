...
export function findItemByAccessKey(
  ...
  for (const item of items) {
    if (itemMayHaveAccessKey(item) && item.accessKey?.toLowerCase() === lowerCaseAccessKey) {
      return item;
    }
  }
  return null;
...
export class AppMenu {
  ...
  public withOpenedMenu(submenuItem: ISubmenuItem, selectFirstItem = false): AppMenu {
    ...
    const parentMenuIndex = this.openMenus.findIndex((menu) => menu.items.includes(ourMenuItem));
    ...
    const selectedItem = selectFirstItem
      ...
      : undefined;
    newOpenMenus.push(selectedItem ? { ...ourMenuItem.menu, selectedItem } : ourMenuItem.menu);
  ...
  public withClosedMenu(menu: IMenu) {
    ...
    return new AppMenu(this.menu, this.openMenus.slice(0, index), this.menuItemById);
  }
  ...
  public withLastMenu(menu: IMenu) {
    ...
    return new AppMenu(this.menu, this.openMenus.slice(0, index + 1), this.menuItemById);
  }
  ...
  public withDeselectedMenu(menu: IMenu) {
    const ourMenuIndex = this.openMenus.findIndex((candidate) => candidate.id === menu.id);
    if (ourMenuIndex === -1) {
      return this;
    }
