...
describe("Repository", () => {
  ...
  describe("getUpdateBranchStrategy", () => {
    it("defaults to merging when the preference is unset", () => {
      ...
      assert.equal(getUpdateBranchStrategy(repository), UpdateBranchStrategy.Merge);
    });

    it("returns rebase when the repository is configured to rebase", () => {
      const repository = new Repository("/some/path", -1, null, false, null, null, null, {
        updateBranchStrategy: UpdateBranchStrategy.Rebase,
      });
      assert.equal(getUpdateBranchStrategy(repository), UpdateBranchStrategy.Rebase);
    });
