import { API, IAPIRepository, IAPIIdentity, IAPIEmail } from "../lib/api";
...
export function createMockAPI(overrides: APIMethodOverrides = {}): API {
  const api = new API("https://api.github.com", "mock-token-for-testing", "mock-login-for-testing");
  ...
  return new Proxy(api, {
    get(target, property, receiver) {
      ...
      return () =>
        Promise.reject(new Error(`No mock implementation registered for API.${property}`));
    },
  });
}
 ...
 */
export function createMockAPIRepository(overrides: Partial<IAPIRepository> = {}): IAPIRepository {
  return {
 ...
 */
export function createMockAPIIdentity(overrides: Partial<IAPIIdentity> = {}): IAPIIdentity {
  return {
    id: 1,
    login: "octocat",
