...
describe("current window controls", () => {
  ...
  it("stores and clears the selected repository through the window-scoped command", async () => {
    ...
    expect(invoke).toHaveBeenNthCalledWith(1, "set_window_selected_repository", {
      repositoryPath: "/repo/../repo",
    });
    expect(invoke).toHaveBeenNthCalledWith(2, "set_window_selected_repository", {
      repositoryPath: null,
    });
  });
  ...
  const stateCases: ReadonlyArray<
    readonly [
      WindowState,
      Partial<Record<"fullscreen" | "maximized" | "minimized" | "visible", boolean>>,
    ]
  > = [
  ...
  it.each(stateCases)("reports %s using the upstream state precedence", async (expected, state) => {
  ...
  it("does not retain a state callback when native subscription fails", async () => {
    ...
    await expect(onWindowStateChanged(callback)).rejects.toThrow("listener failed");
  ...
  it("unwraps per-window zoom events", async () => {
    ...
    expect(currentWindow.listen).toHaveBeenCalledWith("zoom-factor-changed", expect.any(Function));
  ...
  it("unwraps launch timing events using the domain model", async () => {
    ...
    currentWindow.listen.mockImplementation(
      async (_name: string, handler: (event: { payload: ILaunchStats }) => void) => {
    ...
    expect(currentWindow.listen).toHaveBeenCalledWith("launch-timing-stats", expect.any(Function));
