 ...
 */
export function setWindowSelectedRepository(repositoryPath: string | null): Promise<void> {
...
export function openRepositoryInNewWindow(repositoryPath: string): Promise<void> {
 ...
 */
export function onWindowFocusChanged(callback: (focused: boolean) => void): Promise<UnlistenFn> {
...
export function sendReady(rendererReadyTime: number): Promise<OpenRepositoryAction | null> {
  return invoke("renderer_ready", { rendererReadyTime });
}

export function onLaunchTimingStats(callback: (stats: ILaunchStats) => void): Promise<UnlistenFn> {
