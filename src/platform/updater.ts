...
export class UpdateController {
  ...
  public get isCloseBlocked(): boolean {
    return this.currentState.status === "downloading" || this.currentState.status === "installing";
  }
    ...
    switch (event.event) {
      ...
      case "Progress":
        this.setState({
          ...this.currentState,
          status: "downloading",
          downloadedBytes: this.currentState.downloadedBytes + event.data.chunkLength,
        });
        break;
      case "Finished":
        break;
...
export function onAutoUpdaterCheckingForUpdate(listener: () => void): Unsubscribe {
...
export function onAutoUpdaterUpdateAvailable(listener: () => void): Unsubscribe {
  return applicationUpdateController.onDidChange((state) => {
...
export function onAutoUpdaterUpdateNotAvailable(listener: () => void): Unsubscribe {
  return applicationUpdateController.onDidChange((state) => {
...
export function onAutoUpdaterUpdateDownloaded(listener: () => void): Unsubscribe {
  return applicationUpdateController.onDidChange((state) => {
