...
pub async fn show_contextual_menu<R: Runtime>(
    ...
    // muda's popup call blocks until selection or dismissal, but Tauri forwards
    // the selected item through its event-loop proxy. Queueing this marker after
    // popup returns lets that earlier MenuEvent win; otherwise the marker closes
    // a dismissed menu with `None`.
