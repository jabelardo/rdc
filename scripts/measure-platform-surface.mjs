...
const CONSUMER_OUTSIDE_PROXY = new Map([
  ...
  ["getKeybindings", "Phase 4 menu bootstrap; new capability with no upstream proxy entry point"],
  [
    "setKeybinding",
  ...
  ["setNativeMenu", "macOS startup installs the frontend-owned default tree after renderer load"],
  [
  ...
  for (const { file, source } of files) {
    ...
    const visit = (node) => {
      if (
        ...
        if (
          channel &&
          (ts.isStringLiteral(channel) || ts.isNoSubstitutionTemplateLiteral(channel))
        ) {
...
function measure() {
  ...
  const implementedPhase4Exports = phase4Exports.filter(
    (name) => providedExports.has(name) || providedExports.has(PROXY_ADAPTERS.get(name)),
  );
