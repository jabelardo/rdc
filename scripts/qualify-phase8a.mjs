...
export function qualifyPhase8aInputs(root) {
  ...
  const capability = readJson(path.join(root, "src-tauri", "capabilities", "default.json"));
  ...
  requireCondition(
    tauriConfig.bundle?.active === true && tauriConfig.bundle?.targets === "all",
  ...
  requireCondition(
    capability.windows?.includes("main") && capability.windows?.includes("repository-*"),
