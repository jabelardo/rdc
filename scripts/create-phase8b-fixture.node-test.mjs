  ...
  try {
    ...
    assert.equal(
      git(
        ...
        git(scenarios.populated.repository, 'rev-parse', 'HEAD'),
      false
    )
    assert.equal(
      git(
    ...
    assert.equal(
      readFileSync(
        path.join(scenarios.lineDiscard.repository, scenarios.lineDiscard.file),
        'utf8'
      ),
    ...
    assert.throws(
      ...
      error => {
        ...
        assert.match(
          String(error.stderr),
          new RegExp(scenarios.commitHook.expectedHookMessage)
        )
        return true
      }
    )
    ...
    assert.equal(
      git(
        scenarios.mergeConflict.repository,
        'diff',
        '--name-only',
        '--diff-filter=U'
      ),
    ...
    assert.equal(
      existsSync(
        path.join(scenarios.mergeConflict.repository, '.git', 'MERGE_HEAD')
      ),
      true
    )
    ...
    assert.equal(
      git(
        scenarios.remoteFetchPull.repository,
        'rev-parse',
    ...
    const delayHook = path.join(
      scenarios.delayedPush.remote,
      'hooks',
      'pre-receive'
    )
    ...
    assert.equal(
      git(
        scenarios.unreachableRemote.repository,
        'remote',
        'get-url',
        'origin'
      ),
