...
through the command registry remain as the final evidence gate. Real command-layer Rebase and
Cherry-pick recovery tests now prove conflict abort restores the branch/worktree and releases the
registry lock. Cherry-pick recovery explicitly handles the marker-only state used by a single
conflicted pick, which has `CHERRY_PICK_HEAD` without a sequencer snapshot. Fixture
